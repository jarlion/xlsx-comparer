...
export class InitOptions {
  ...
  parse(params: any): this {
    ...
    if (!key)
      throw new Error(`> !!! 请指定用于对确定行数据的字段序号，或列名 ${key}`);
  ...
  initDisplayColumns(head: any): string[] {
    ...
    this.displayColumns = Array.from(columns).map((c) =>
      this._getColumnName(head, c)
    );
  ...
  pickProps(data: any[]): any[] {
    return data.map((d, index) =>
      pick(d, this.displayColumns, { $rank: index + 1 })
    );
  }
  ...
  validate(rows: any[], xlsxPath: string): this {
    ...
    if (index !== -1)
      throw new Error(
        `> !!! ${xlsxPath} 的 ${this.key} 列, 第 ${index} 行重复`
      );
    return this;
  }
}
